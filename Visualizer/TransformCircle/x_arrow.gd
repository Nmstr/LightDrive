...
func _process(delta: float) -> void:
	...
	if abs(drag_distance) > 0.0:
		fixture.move_and_collide(Vector3(drag_distance * delta, 0, 0))
		fixture.get_parent().get_node("Hud/FixturePropertyEditor/Background/FixturePropertyVBox/PositionHBox/XPosSpin").value = fixture.position.x
		drag_distance = 0.0
...
func _input(event) -> void:
	...
	if mouse_click and mouse_click.button_index == MOUSE_BUTTON_LEFT and not mouse_click.pressed:
		...
		drag_distance += event.position.distance_to(last_position) * event.position.direction_to(last_position).x * -1
